...
test('pulling after a push returns the just-pushed blob', async () => {
  ...
  await handlePushBlob({ accountId: 1, baseVersion: 0, envelopeVersion: 1, ciphertext, shrinkAcknowledged: false, now: NOW }, storage);
...
test('a pull for account 2 never sees account 1 data', async () => {
  ...
  await handlePushBlob(
    { accountId: 1, baseVersion: 0, envelopeVersion: 1, ciphertext: new TextEncoder().encode('account 1') , shrinkAcknowledged: false, now: NOW },
    storage,
  );
